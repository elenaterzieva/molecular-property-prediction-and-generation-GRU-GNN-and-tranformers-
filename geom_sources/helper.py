...
def get_log_dnn(in_feats, out_feats, activation_factory=nn.SiLU, n_layers=3):
    layers = []
    for _ in range(n_layers):
        layers.append(Linear(in_feats, in_feats))
        layers.append(activation_factory())
    layers.append(Linear(in_feats, out_feats))
...
class Jastrow(nn.Module):
    def __init__(self, embedding_dim, *, n_layers=3, sum_first=True):
        super().__init__()
        self.net = get_log_dnn(embedding_dim, 1, nn.SiLU, n_layers=n_layers)
